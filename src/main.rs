...
impl TN5250RApp {
    ...
    fn do_connect(&mut self) {
        ...
        if let Err(e) = self.controller.connect_async_with_tls_options(self.host.clone(), self.port, Some(use_tls), Some(insecure), ca_opt) {
            ...
            monitoring.integration_monitor.record_integration_event(monitoring::IntegrationEvent {
                timestamp: std::time::Instant::now(),
                event_type: monitoring::IntegrationEventType::IntegrationSuccess,
                source_component: "main".to_string(),
                target_component: Some("network".to_string()),
                description: format!("Successfully initiated connection to {}:{}", self.host, self.port),
                details: std::collections::HashMap::new(),
                duration_us: None,
                success: true,
            });
        ...
        if self.connecting {
            ...
            if let Some(err) = self.controller.take_last_connect_error() {
                ...
                let alert_level = if err.to_lowercase().contains("timed out") {
                    ...
                    monitoring::AlertLevel::Critical
                };
                ...
                monitoring.integration_monitor.record_integration_event(monitoring::IntegrationEvent {
                    timestamp: std::time::Instant::now(),
                    event_type: monitoring::IntegrationEventType::IntegrationFailure,
                    source_component: "network".to_string(),
                    target_component: Some("controller".to_string()),
    ...
    fn show_monitoring_dashboard_ui(&mut self, ui: &mut egui::Ui) {
    ...
    fn refresh_monitoring_reports(&mut self) {
        let monitoring = monitoring::MonitoringSystem::global();
    ...
    fn generate_full_monitoring_report(&mut self) {
        let monitoring = monitoring::MonitoringSystem::global();
        let report = monitoring.generate_report();
