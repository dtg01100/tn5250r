...
fn test_live_eor_negotiation() {
    println!("\n{}", "=".repeat(70));
    println!("LIVE TEST: EOR Negotiation with pub400.com");
    println!("{}", "=".repeat(70));
    
    let server = "pub400.com";
    let port = 23;
    ...
    println!("\nSending initial negotiation ({} bytes)...", initial.len());
    if let Err(e) = stream.write_all(&initial) {
    ...
    loop {
        round += 1;
        if round > 10 {
            println!("Max rounds reached");
            break;
        }
        
        match stream.read(&mut buffer) {
            Ok(n) if n > 0 => {
                println!("\nRound {}: Received {} bytes", round, n);
                println!("  Raw: {:02X?}", &buffer[..n.min(32)]);
                
                // Show what server is requesting
                let mut i = 0;
                while i + 2 < n {
                    if buffer[i] == 255 { // IAC
                        let cmd = buffer[i + 1];
                        ...
                        println!("    Server: IAC {} {} (0x{:02X})", cmd_name, opt_name, opt);
                        i += 3;
                    } else {
                        i += 1;
                    }
                ...
                if !response.is_empty() {
                    ...
                    while i + 2 < response.len() {
                        if response[i] == 255 {
                            let cmd = response[i + 1];
                            let opt = response[i + 2];
                            let cmd_name = match cmd {
                                251 => "WILL",
                                252 => "WONT",
                                253 => "DO",
                            ...
                            println!("    Client: IAC {} {} (0x{:02X})", cmd_name, opt_name, opt);
                            i += 3;
                        } else {
                            i += 1;
                        }
    ...
    println!("\nDetailed State:");
    for (opt, state) in negotiator.get_negotiation_state_details() {
        println!("  {:?}: {:?}", opt, state);
    }
}